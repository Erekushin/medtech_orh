...
class AuthController extends GetxController {
  ...
  Future geregeUserLogin(Function retryFunction) async {
    ...
    GlobalHelpers.pass =
        md5.convert(utf8.encode(loginPass.text.toString())).toString();
    if (GlobalHelpers.userName.isNotEmpty && GlobalHelpers.pass.isNotEmpty) {
      var data = await GlobalHelpers.postRequestGeneral.getdata(loginBody(),
          "110002", UriAdresses.medCore); //UriAdresses.geregeUserLoginUri
      print('gerege login ajillah yostoi');
      log(jsonEncode(loginBody()));
      ereklog.wtf(data);
      user = UserInfo.fromJson(jsonDecode(data.toString()));
      switch (user.code) {
        case 200:
          loginloading.value = false;
          // GlobalHelpers.auth =  'bearer ' + geregeUser.result!.token!.token!;
          Get.find<ImageController>().imageBytes.value =
              user.result!.picture!.cast<int>();
          await surveyController.listGet(RouteUnits.home, '120002',
              user.result!.userId!, '', user.result!.phone!);
          await surveyController.listGet('/segmented', '120009',
              user.result!.userId!, '', user.result!.phone!);
          retryFunction();
          break;
        case 403:
          loginloading.value = false;
          Get.snackbar('Хэрэглэгч бүртгэлгүй байна',
              'Бүртгэл үүсгэхийн тулд Gerege App ыг суулгана уу!',
      ...
      loginloading.value = false;
      Get.snackbar('Талбаруудын утга хоосон байна',
          "Хэрэглэгчийн нэр ихэвчлэн Регистрийн дугаараар баталгаажсан байгаа!",
          snackPosition: SnackPosition.BOTTOM,
          colorText: Colors.white,
          backgroundColor: Colors.grey[900],
          margin: const EdgeInsets.all(5));
  ...
  Future registration() async {
    var data = await GlobalHelpers.postRequestGeneral
        .getdata(registerInfo(), '110001', UriAdresses.medCore);
    print(data);
    print(registerInfo());
    ...
    switch (registerResponse.code) {
      case 200:
        Get.snackbar('Gerege Medtech family-д тавтай морил',
            "Хэрэглэгчийн бүртгэл амжилттай үүслээ",
            snackPosition: SnackPosition.BOTTOM,
            colorText: Colors.white,
            backgroundColor: Colors.grey[900],
            margin: const EdgeInsets.all(5));
        switch (registerResponse.result!.code) {
          case 200:
            Get.snackbar('', "Регистрийн дугаарыг амжилттай шалгалаа",
                snackPosition: SnackPosition.BOTTOM,
                colorText: Colors.white,
                backgroundColor: Colors.grey[900],
                margin: const EdgeInsets.all(5));
          ...
          case 400:
            Get.snackbar('',
                "регистрийн дугаар буруу байж болзошгүй байна. Profile хэсэг дээр мэдээллийг засаж оруулна уу",
                snackPosition: SnackPosition.BOTTOM,
                colorText: Colors.white,
                backgroundColor: Colors.grey[900],
                margin: const EdgeInsets.all(5));
      ...
      case 400:
        Get.snackbar(
            'Бүртгэл үүсгэж чадсангүй', registerResponse.message.toString(),
            snackPosition: SnackPosition.BOTTOM,
            colorText: Colors.white,
            backgroundColor: Colors.grey[900],
            margin: const EdgeInsets.all(5));
  ...
  Future updateProPic() async {
    var data = await GlobalHelpers.postRequestGeneral
        .getdata(updateProbody(), '110003', UriAdresses.medCore);
    GForUpdateProPic = EResponse.fromJson(jsonDecode(data.toString()));
    switch (registerResponse.code) {
      case 200:
        Get.snackbar('', "Зураг амжилттай солигдлоо",
            snackPosition: SnackPosition.BOTTOM,
            colorText: Colors.white,
            backgroundColor: Colors.grey[900],
            margin: const EdgeInsets.all(5));
