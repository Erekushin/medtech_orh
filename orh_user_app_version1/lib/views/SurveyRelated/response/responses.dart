  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      itemBuilder: (c, i){
        ...
        return   InkWell(onTap: (){resCont.answers(c!);},
                         child: Container(
          ...
          Visibility(visible: resCont.surveyResponses.result == null? true : false,
...
class _ResStaState extends State<ResSta> {
  ...
  void initState() {
      ...
      for(int a = 0; a<resCont.statisticAnswer.result!.length; a++){
        for(int w = 0; w < resCont.statisticAnswer.result![a].answers!.length; w++){
           if(resCont.statisticAnswer.result![a].answers![w].numberAnswer! > maxYvalue){
            maxYvalue = resCont.statisticAnswer.result![a].answers![w].numberAnswer!.toDouble();
           }
           if(resCont.statisticAnswer.result![a].answers![w].numberAnswer! < minYvalue){
 ...
 SfCartesianChart _buildLineZoneChart(BuildContext context) {
  ...
  List<CartesianSeries<_ChartData, String>> getlines() {
    ...
    for(int q = 0; q < resCont.statisticAnswer.result![0].answers!.length;  q++){
  ...
  List<_ChartData> getData(int index) {
    ...
    for (int i = 0; i < resCont.statisticAnswer.result!.length; i++) {
      data.add(_ChartData(resCont.statisticAnswer.result![i].answers![index].createdDate!.toString().substring(6,18), 
                          resCont.statisticAnswer.result![i].answers![index].numberAnswer!.toDouble(),));
    }
    return data;
  }
}
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.builder(
                    itemCount: resCont.statisticAnswer.result![0].answers!.length,
                    itemBuilder: (c,i){
                      var item  = resCont.statisticAnswer.result![0].answers![i];
